...
FEAT-001 读取路径同时执行 `BE-PERF-READ-001..006`：

- `BE-PERF-READ-001`：不得对随历史增长的查询直接 `.all()`/`list()`；必须先在数据库
  聚合到固定区间，或应用服务端硬上限。
- `BE-PERF-READ-002`：不得只为 count/bucket/sum/min/max 把时间戳或 ORM 对象加载到
  Python；聚合由数据库完成，应用只物化有界结果。
- `BE-PERF-READ-003`：游标排序必须是全序，包含唯一 tie-breaker，并绑定 family、资源、
  筛选、排序与快照；游标不参与授权，错 scope 必须拒绝。
- `BE-PERF-READ-004`：offset 只可作为已记录的兼容合同；新消费者使用 cursor，且两种
  方式都必须执行最大页长。
- `BE-PERF-READ-005`：新增/修改列表必须测试默认/最大页、空页/末页、非法游标、租户隔离、
  并发插入/更新语义、SQL 数量和响应上限。
- `BE-PERF-READ-006`：索引是读取合同的一部分；必须验证 SQLite 与 MySQL 的真实执行计划，
  不能根据 ORM 声明推断索引已生效。
