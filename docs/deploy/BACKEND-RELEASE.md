...
三个模板都是**一次性订阅**（后台该类目当前只提供一次性模板，「选用模板」里没有长期模板可选），
所以每条提醒都要消耗一次授权额度。运行时按这个前提工作：家长每同意一次就攒下一条额度（可重复攒），
发出一条扣一条，扣到 0 时授权置 `expired`，「提醒设置」页会提示再存。小程序还会在家长的自然动作
（完成复习反馈、保存日程、确认入库）里按额度自动请求续订，但弹窗仍必须由家长同意，服务端不能代劳。
`long_term` 必须保持 `false`；只有后台真的出现同题材长期模板时才改为 `true`。
长期订阅只向政务民生/医疗/交通/金融/教育等线下公共服务类目开放，不要为了推送体验去改服务类目。

#### 5.1.2 订阅事件回调（可选但强烈建议）

不配置时 `GET|POST /api/v1/notifications/wechat/events` 整体返回 404，服务照常运行，只是云端不知道家长
在微信里做了什么：在「服务通知」里拒收不会被感知，异步送达失败也不会反映到投递记录。

```text
【需要人工操作】配置微信消息推送，接收订阅事件
原因：消息推送 URL 与 Token 由微信后台配置并参与验签，Token 属于运行时密钥，不能进入仓库、CLI 历史或聊天。
操作入口：https://mp.weixin.qq.com/ （小程序后台）与 https://cloud.weixin.qq.com/ （云托管服务设置）
导航路径：小程序后台 → 开发管理 → 开发设置 → 消息推送；prod-d2g14rwoycac6b45d → 云托管 → flask-ik19 → 版本配置
需要填写/选择：
- 服务器地址（URL）：<后端公网地址>/api/v1/notifications/wechat/events
- 令牌（Token）：自行生成的随机值（可用 uv run python tools/notification_config.py secret 生成后截取），
  同一个值同时填入微信后台与云托管环境变量 PUSH_KIDS_WECHAT_MESSAGE_TOKEN。
- 消息加解密方式：明文模式（当前实现只做签名校验，不做 AES 解密）。
- 数据格式：JSON 或 XML 均可，两种都已支持。
决策标准：Token 只用于验签，与 PUSH_KIDS_NOTIFICATION_SECRET_KEY 必须是两个不同的值，不要复用。
完成标志：微信后台「提交」时 URL 校验通过（服务返回 echostr）；家长在服务通知里关掉提醒后，
GET /api/v1/notifications/settings 对应类别在下一次读取时变为「微信里已拒收」。
完成后回复：已完成消息推送配置
```

回调的安全边界：验签失败返回 403、未配置 Token 返回 404、body 超过 16KB 或无法解析返回 400；
匹配不到已存在的接收标识或未配置的模板时不改任何状态；日志只记录事件类型与是否更新，不记录 OpenID。
